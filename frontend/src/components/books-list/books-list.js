...
export const BooksList = () => {
  ...
  const [categories, setCategories] = useState(["Sci-Fi", "Action", "Adventure", "Horror", "Romance", "Mystery", "Thriller", "Drama", "Fantasy", "Comedy", "Biography", "History", "Western", "Literature", "Poetry", "Philosophy"])
  ...
  const changeCategory = async (e) =>{
    ...
    const { books } = await BackendApi.book.getAllBooks({category : e?.target?.value})
    setBooks(books)
  }
  ...
  return (
    <>
      ...
      <div style={{display : "flex", flexDirection : "row", justifyContent: "space-between", marginBottom : "1.5rem"}}><Typography variant="h5">Select Category</Typography> <Button onClick={resetCategory} className={classes.primary}>Reset Category</Button></div> 
      <div>
        {categories.map((category)=>{
        return(<Button onClick={changeCategory} className={ category == selectedCategory ? `${classes.primary}` : ""} value={category}>{category}</Button>)
      })}
      </div>
                  ...
                  {(rowsPerPage > 0
                    ...
                    <TableRow key={book.isbn}>
                      <TableCell component="th" scope="row">
                        ...
                        >
                          <img src={book?.image} alt="book" height="60" />
                        </div>
                      </TableCell>
                      <TableCell component="th" scope="row">
                        {book.name}
