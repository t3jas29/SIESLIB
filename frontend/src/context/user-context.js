...
const UserProvider = ({ children }) => {
    ...
    useEffect(() => {
        ...
        setIsUser(user && user.role === 'guest')
    }, [user])
    ...
    const registerUser = async (username, password) => {
        const { user, error } = await BackendApi.user.register(username, password)
        if (error) {
            NotificationManager.error(error)
        } else {
    ...
    return (
        <UserContext.Provider value={{ user, loginUser,  registerUser, logoutUser, isAdmin, isUser }}>
            {children}
