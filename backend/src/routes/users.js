...
router.post("/borrow", async (req, res, next) => {
  try {
    ...
    book.borrowedBy = book.borrowedBy ?  [...book.borrowedBy, user?.id] : [user?.id]
    console.log(book)
    const response = await BookModel.findOneAndUpdate({ isbn: req.body.isbn }, book)
...
router.post("/register", async (req, res, next) => {
  try {
    ...
    if (user) {
      ...
      return res.status(200).json({ msg: 'User Created Succesfully', data : response })
    }
